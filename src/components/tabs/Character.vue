...
export default {
  ...
  setup() {
    ...
    let currentWeaponLeft = ref(useJigsStore().playerStats.weapon_left);
    ...
    let currentWeaponRight = ref(useJigsStore().playerStats.weapon_right);
  ...
  //    currentWeaponLeft: this.jigs.playerStats.weapon_left,
  //    currentWeaponRight: this.jigs.playerStats.weapon_right
  //  };
  // },
  props: {
    'val': {
  ...
  computed: {
    ...
    selectedWeapon() {
      currentWeaponLeft = this.jigs.playerStats.weapon_left;
      console.log('left: ' + currentWeaponLeft);

      // currentWeaponRight = this.jigs.playerStats.weapon_right;
      // console.log('right: ' + currentWeaponRight)
...
<template>
  <div class="character">
    ...
    <div class="weapon">
      ...
      <div class="weapon weapon-left">
        <label>Weapon Left: {{ currentWeaponLeft }}</label>
          <div v-for="(weapon, i) in weaponsLeft">
            <input
              type="radio"
              :id="weapon.id"
              :name="`select-${weapon.name}`"
              :value="weapon.id"
              v-model=currentWeaponLeft>
            <label :for="weapon.id">{{weapon.name}}</label>
        </div>
      </div>

      <!-- <div class="weapon weapon-right">
        <label>Weapon Right</label>
        <div class="weapon--select">
          <div class="weapon-value">
            <label for="sword">Sword</label>
            <input type="radio" id="sword" value="Sword" :checked="this.jigs.playerStats.weapon_right == 0">
          </div>
          <div class="weapon-value">
            <label for="gun">Gun</label>
            <input type="radio" id="gun" value="Gun" :checked="this.jigs.playerStats.weapon_right == 1 ? true : false">
          </div>
        </div>
      </div> -->
    </div>
